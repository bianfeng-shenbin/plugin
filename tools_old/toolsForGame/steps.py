...
class step2(step):
    ...
    def initStep(self, root, pluginList):
        ...
        requiredBtns = ['cocos2dx']
        for required in requiredBtns:
            var = StringVar()
            self.checkValues.append(var)
            btn = Checkbutton(self.stepFrame, text=required, state="disable", variable=var, onvalue=required, offvalue=required)
            btn.pack(anchor='nw', padx=50)
            self.checkBtns.append(btn)
