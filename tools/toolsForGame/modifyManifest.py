...
def doModify(sourceFile, root):
    ...
    f.close()
	# check config for application atr -------------- add by shen
    atrCfgNode = sourceRoot.find('applicationAtr')
    if atrCfgNode is not None:
        atrNameWord = atrCfgNode.get('{' + androidNS + '}name')
        print 'Modify AndroidManifest.xml for application ' + str(atrNameWord)
        if atrNameWord != None and len(atrNameWord) > 0:
            rootEle = root.find('application')
            rootEle.set('{' + androidNS + '}name', atrNameWord)
